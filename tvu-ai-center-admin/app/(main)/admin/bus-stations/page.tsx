...
import { getBusStationApi, deleteBusStationApi } from '@/apis/admin/bus-station';
...
const UserAdminPage = () => {
    ...
    const renderHeader1 = () => {
        return (
            <div className="flex justify-content-between">
                <Button type="button" icon="pi pi-plus" label="Thêm trạm dừng" outlined onClick={() => router.push('/admin/bus-stations/create')} />
            </div>
        );
    };
    ...
    const getAdminUsers = async () => {
        try {
            ...
            setAdminUsers(response.data.data.reverse());
    ...
    const handleDeleteRole = async () => {
        ...
        const response = await apiClient.delete(deleteBusStationApi(userSelected.id));
    ...
    const actionBody = (rowData: any) => {
        return (
            <div className="flex justify-content-center gap-2">
                {/* <Button icon="pi pi-pencil" className="p-button-rounded p-button-success p-mr-2" onClick={() => router.push(`/admin/bus-stations/${rowData.id}`)} /> */}
            ...
            <div className="col-12">
                <div className="card">
                    ...
                    <DataTable value={adminUsers} paginator className="p-datatable-gridlines" showGridlines rows={10} dataKey="id" loading={loading1} responsiveLayout="scroll" header={header1} emptyMessage="Chưa có dữ liệu">
                        <Column field="name" header="Tên trạm" style={{ minWidth: '12rem' }} />
                        {/* <Column field="phoneNumber" header="Số điện thoại" style={{ minWidth: '12rem' }} /> */}
                        <Column field="fullAddress" header="Địa chỉ" style={{ minWidth: '12rem' }} />
                    ...
                    <Dialog header="Cảnh báo" visible={displayDeleteConfirmation} onHide={() => setDisplayDeleteConfirmation(false)} style={{ width: '350px' }} modal footer={confirmationDialogFooter}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            <span>
                                Bạn chắc chắn muốn xóa trạm <b>{userSelected?.name}</b>?
                            </span>
                        </div>
                    </Dialog>
                </div>
