...
import { createBusRoutenApi } from '@/apis/admin/bus-route';
...
const UserAdminCreate = () => {
    ...
    const showSuccess = () => {
        toast.current.show({ severity: 'success', summary: 'Thành công', detail: 'Thêm tuyến đường thành công', life: 2500 });
    };
    ...
    const save = async (exit = false) => {
        ...
        const response = await apiClient.post(createBusRoutenApi, data);

        if (response.status == 200) {
            if (exit) {
                router.push('/admin/bus-routes');
            } else {
                setSelectBusStations([]);
                showSuccess();
            }
        } else {
            alert('Có lỗi xảy ra');
                        ...
                        <div className="p-fluid formgrid grid">
                            ...
                            <div className="field col-12 md:col-6">
                                ...
                                <div className="p-inputgroup flex-1">
                                    <InputNumber placeholder="Nhập đơn giá" onChange={(e) => setPrice(e.value)} />
